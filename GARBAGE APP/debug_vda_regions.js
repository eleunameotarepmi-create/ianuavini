...
// Note: Keeping this in sync with what we saw in the view_file of constants.tsx
...
// Filter for only VDA wineries or unknown ones (exclude Piemonte)
const piemonteRegionIds = ['alto-piemonte', 'canavese', 'monferrato', 'tortonese', 'roero', 'langhe'];
...
}).filter(w => !piemonteRegionIds.includes(w.calculatedRegion)); // Keep VDA and unknown

console.log("=== WINERIES ANALYSIS ===");
problematicWineries.forEach(w => {
    // Only print interesting ones (unknown, or mismatches, or 0 altitude)
    if (w.calculatedRegion === 'unknown' || w.calculatedAltitude === 0 || w.manualRegion && w.manualRegion.toLowerCase() !== w.calculatedRegion) {
        ...
        console.log(`Manual Region (DB): ${w.manualRegion}`);
        console.log(`Calculated Region: ${w.calculatedRegion}`);
        console.log(`Calculated Altitude: ${w.calculatedAltitude}`);
    }
});
