...
// Find Dish
// Try multiple variations if needed for finding the dish
const dish = data.menu.find(m => m.name.toLowerCase().includes('valpellinentse') || m.name.toLowerCase().includes('seupa'));
console.log("=== DISH ===");
console.log(dish);
...
if (dish && wine) {
    ...
    console.log("Could not find both items to test logic.");
}
